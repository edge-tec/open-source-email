...
<div style="padding:2rem;max-width:800px;margin:0 auto;overflow-y:auto;height:100%">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
            <div>
                ...
                <div style="font-size:.85rem;line-height:1.6;color:var(--t1)">
                    <strong>Server:</strong> <?php echo e(config('edgemail.hostname')); ?><br>
                    <strong>Port:</strong> 993<br>
                    <strong>Encryption:</strong> SSL/TLS<br>
                    <strong>Username:</strong> <?php echo e($mailbox->email); ?><br>
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;box-shadow:var(--shadow)">
        ...
        <form method="POST" action="<?php echo e(route('webmail.settings.update')); ?>">
            <?php echo csrf_field(); ?>
            <div class="form-group" style="margin-bottom:1rem;">
                <label class="form-label">Display Name (Sender Name)</label>
                <input type="text" name="name" class="form-control" value="<?php echo e($mailbox->name); ?>" placeholder="John Doe">
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-bottom:1.5rem;box-shadow:var(--shadow)">
        ...
        <form method="POST" action="<?php echo e(route('webmail.settings.update')); ?>">
            ...
            <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:1rem">
                <input type="checkbox" name="autoresponder_active" value="1" id="ar_active" <?php echo e($autoresponder && $autoresponder->status === 'active' ? 'checked' : ''); ?> style="accent-color:var(--accent)">
                <label for="ar_active" style="font-weight:500">Enable Autoresponder</label>
            </div>
            
            <div class="form-group">
                <label class="form-label">Subject</label>
                <input type="text" name="autoresponder_subject" class="form-control" value="<?php echo e($autoresponder->subject ?? 'Out of office'); ?>">
            </div>
            
            <div class="form-group">
                <label class="form-label">Message Body</label>
                <textarea name="autoresponder_body" class="form-control" rows="4"><?php echo e($autoresponder->body ?? "I am currently away and will reply as soon as possible."); ?></textarea>
            </div>
            
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
                <div class="form-group">
                    <label class="form-label">Start Date (Optional)</label>
                    <input type="date" name="autoresponder_start" class="form-control" value="<?php echo e($autoresponder->start_date ?? ''); ?>">
                </div>
                <div class="form-group">
                    <label class="form-label">End Date (Optional)</label>
                    <input type="date" name="autoresponder_end" class="form-control" value="<?php echo e($autoresponder->end_date ?? ''); ?>">
                </div>
            </div>
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;box-shadow:var(--shadow)">
        ...
        <form method="POST" action="<?php echo e(route('webmail.settings.update')); ?>">
            <?php echo csrf_field(); ?>
            <?php $rule = $forwarding->first(); ?>
            <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:1rem">
                <input type="checkbox" name="forwarding_active" value="1" id="fwd_active" <?php echo e($rule && $rule->status === 'active' ? 'checked' : ''); ?> style="accent-color:var(--accent)">
                <label for="fwd_active" style="font-weight:500">Enable Forwarding</label>
            </div>
            
            <div class="form-group">
                <label class="form-label">Forward To Address</label>
                <input type="email" name="forwarding_destination" class="form-control" placeholder="external@gmail.com" value="<?php echo e($rule->destination ?? ''); ?>">
            </div>
            
            <div style="display:flex;align-items:center;gap:.5rem;margin-bottom:1rem">
                <input type="checkbox" name="forwarding_keep_copy" value="1" id="fwd_keep" <?php echo e($rule && $rule->keep_copy ? 'checked' : ''); ?> style="accent-color:var(--accent)">
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:1.5rem;margin-top:1.5rem;box-shadow:var(--shadow)">
        ...
        <?php if($filters && $filters->count() > 0): ?>
                    ...
                    <tbody>
                        <?php $__currentLoopData = $filters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $filter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr style="border-bottom:1px solid var(--border);">
                                <td style="padding:0.5rem"><?php echo e($filter->name); ?></td>
                                <td style="padding:0.5rem">If <?php echo e($filter->criteria_field); ?> <?php echo e($filter->criteria_operator); ?> "<?php echo e($filter->criteria_value); ?>"</td>
                                <td style="padding:0.5rem"><?php echo e(ucfirst(str_replace('_', ' ', $filter->action))); ?> <?php echo e($filter->action_value ? '-> ' . $filter->action_value : ''); ?></td>
                                <td style="padding:0.5rem;">
                                    <form method="POST" action="<?php echo e(route('webmail.settings.filter.destroy', $filter->id)); ?>">
                                        <?php echo csrf_field(); ?>
        ...
        <form method="POST" action="<?php echo e(route('webmail.settings.filter.add')); ?>">
            <?php echo csrf_field(); ?>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem">
                <div class="form-group">
...
<?php echo $__env->make('webmail.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/webmail/settings.blade.php ENDPATH**/ ?>